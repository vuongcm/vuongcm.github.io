(function() {
	...
	$.fn.hc_countdown = function(options){
	    ...
	    var count = setInterval(function() {
	    	...
	    	this_.html( '<div class="item"><span>'+ days +'</span> ngĂ y</div>' 
	    		+'<div class="item"><span>'+ hours + '</span> giá»</div>' 
	    		+'<div class="item"><span>'+ minutes + '</span> phĂºt </div>'
	    		+'<div class="item"><span>'+ seconds + '</span> giĂ¢y </div>'
	    	);
	    	if (distance < 0) {
	    		clearInterval(count);
	    	}
...
function initMap() {
    ...
    if(m){
        let array = m.getAttribute("data-latlng").split(",");
        let latlng = {lat: Number(array[0]) ,lng: Number(array[1])}
        let map = new google.maps.Map(m, {
            zoom: 17,
            center: latlng,
        });
    ...
    var ctm = document.getElementById('ct-map');
    if(ctm){
        let array = ctm.getAttribute("data-latlng").split(",");
        let latlng = {lat: Number(array[0])  ,lng: Number(array[1])}
        let map = new google.maps.Map(ctm, {
            zoom: 17,
            center: latlng,
        });
...
window.addEventListener('load',function(){
    ...
    script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyCH6whmz7Lxkbi-6h_ruIZK1OKg9GFf8Lo&callback=initMap';
