$(document).ready(function(){
    ...
    //Popup flash sale close
    $('#modal-sale .close').click(function(){
        $('#modal-sale').fadeOut();
        setCookie("popup_flash_sales","1",1);
        //$('body').removeClass('modal-open').css('padding','');
    });
    if($('#plexible-date').length){
        $(`#checkdate`).click(function(){
            if (this.checked){
                $(`#date-sale19-1`).css('display','none');
                $(`#date-sale19-2`).css('display','block');
            }
            else{
                $(`#date-sale19-2`).css('display','none');
                $(`#date-sale19-1`).css('display','block');
            }
        });
        $('#date-sale19-1').datepicker({
            //inline: true,
            minDate:new Date(),
    ...
    if($('.phone-wrap').length){
		...
		if ($("#phonenumber-1").length > 0) {
        $("#phonenumber-1").intlTelInput({
            preferredCountries: ['us', 'gb','au'],
        });
			$("#phonenumber-1").intlTelInput("setNumber", "+1 ");//Phone Number
			let inputTell = document.getElementById('phonenumber-1');
			inputTell.onclick = inputTell.onkeydown = function(event) {
				if(inputTell.value.length<2 || inputTell.value=='+1 '){
					  inputTell.value='+';
				}
				var key = event.keyCode || event.charCode;
				if( key == 8){
					if(inputTell.value.length<3 || inputTell.value=='+1 '){
					   inputTell.value='++';
					}
				}else if(key == 46 ){
					if(inputTell.value.length<2 || inputTell.value=='+1 '){
		...
		if ($("#phonenumber-2").length > 0) {
        $("#phonenumber-2").intlTelInput({
            preferredCountries: ['us', 'gb','au'],
        });
        $("#phonenumber-2").intlTelInput("setNumber", "+1 ");//Phone Number
        let inputTell2 = document.getElementById('phonenumber-2');
        inputTell2.onclick = inputTell2.onkeydown = function(event) {
            if(inputTell2.value.length<2 || inputTell2.value=='+1 '){
                  inputTell2.value='+';
            }    
            var key = event.keyCode || event.charCode;
            if( key == 8){   
                if(inputTell2.value.length<3 || inputTell2.value=='+1 '){ 
                   inputTell2.value='++';
                } 
            }else if(key == 46 ){   
                if(inputTell2.value.length<2 || inputTell2.value=='+1 '){ 
    ...
    /*-------------filter-day---------------*/
    if($('.filter-day-month').length){
        $('#filter-day-cr').datepicker({
            //inline: true,
            minDate:new Date(),
            showOtherMonths: true
        });
        ...
        $('.filter-day-month>form>div:last-child input').click(function(){
            ...
            $(this).next('.box-mon-time').css('display','flex');
        });
        btnMonth.click(function(){
            let thisText = $(this).text();
            btnMonth.removeClass('active');
            $(this).addClass('active');
            $(this).closest('.filter-day-month>form>div:last-child').children('input').val(thisText);
            $('.box-mon-time').css('display','none');
            ajax_date_flash_deals(thisText);
        });
        $('body').click(function(e){
            if(!$('.filter-day-month>form>div:last-child').is(e.target) && $('.filter-day-month>form>div:last-child').has(e.target).length === 0){
                $('.box-mon-time').css('display','none');
            }
        });
    }
    ...
    $('.language .sub-menu-3>li>a').on('click',function(){
        ...
        if (elementTargetId != 'langVN') {
            swapLanguage(elementTargetId);
            setCookie('lang',$('.language>a>span').html(),1);
        }
    });
    $('.language-footer>li>a').on('click',function(){
        let elementTargetId_2 = $(this).attr('id').slice(0,-3);
        if (elementTargetId_2 != 'langVN') {
            swapLanguage(elementTargetId_2);
            setCookie('lang',$('.language>a>span').html(),1);
        }
    });
    ...
    $(".wrap-slide-st7 .gallery").owlCarousel({
        items: 1,
        singleItem: true,
        slideSpeed: 5000,
        useTransform: false,
...
function check_empty(obj){$("#"+obj).css("border","1px solid #ccc");$("#err_"+obj).hide();if($("#"+obj).val()==""){$("#"+obj).css("border","1px solid #FF6600");$("#err_"+obj).show();return false;}}
function check_date(obj){$("#"+obj).css("border","1px solid #ccc");$("#err_"+obj).hide();if($("#"+obj).val()==""){$("#"+obj).css("border","3px solid red");$("#err_"+obj).show();return false;}}
...
function check_flexible(obj){$("#"+obj).css("border","3px solid #ccc");$("#err_"+obj).hide();if($("#"+obj).val()==""){$("#"+obj).css("border","3px solid red");$("#err_"+obj).show();return false;}}
...
function quickcontq_popup(){
    ...
    phone=document.getElementById('phonenumber-2').value;
	...
	if(ques==""||ques=="Type your request here..."){
        ...
        $('#f_question').css("border", "1px solid #ccc");
    }
	if(name==""||name=="Name"){
        ...
        $('#f_name').css("border", "1px solid #ccc");
    }
	mail=check_email2('f_mail');
	if(mail==false){
		$('#f_mail').css("border", "1px solid red");
		//$("#message").html("<font style='color:red; font-size:14px;'>&nbsp;&nbsp;&nbsp;Please type your <b>email</b>.Thanks!</font>");
		return;
	}else {
        $('#f_mail').css("border", "1px solid #ccc");
    }
$(".signin").html("<div style='width: 40px;height: 40px;margin: auto; border-right: 3px solid #59c45a;border-bottom: 3px solid transparent;border-radius: 100%;-webkit-animation: spin .9s linear infinite;-moz-animation: spin .9s linear infinite;-o-animation: spin .9s linear infinite;animation: spin .9s linear infinite;'></div><div style='width:100%;float: left;text-align: center;'><p>Please kindly wait a moment...</p></div>");$.ajax({type:"GET",url:SITE_URL+"/ajax-process.php?mod=popup&fullname="+name+"&email="+email+"&phone="+phone+"&ques="+ques,success:function(xml){var arr_html=xml.split('|');if(arr_html[1]=="ok"){window.location = SITE_URL+'/success.html';/*$(".signin").html(arr_html[0]);*/}else{$("#message").html(arr_html[0]);}},error:function(){}});}
var check_show_fs_popup = false;
function quickcontq_popup_sale(){
	name=document.getElementById('fs_name').value;
	email=document.getElementById('fs_mail').value;
	ques=document.getElementById('fs_question').value;
    ques = encodeURIComponent(ques);
	phone=document.getElementById('phonenumber-1').value;
    if(document.getElementById("checkdate").checked==true){
        var datepicker = check_flexible("date-sale19-2");
        if(datepicker != false){
            datepicker = $('#date-sale19-2').val();
        }else{
            datepicker = "";    
			return;
        }
    }else{
        var datepicker = check_date("date-sale19-1");
        if(datepicker != false){
            datepicker = $('#date-sale19-1').val();
        }else{
            datepicker = "";  
			return;
        }
	...
	if(name==""||name=="Name"){
		$('#fs_name').css("border", "3px solid red");
		return;
	}else {
        $('#fs_name').css("border", "1px solid #ccc");
    }
	mail=check_email2('fs_mail');
	if(mail==false){
		$('#fs_mail').css("border", "3px solid red");
		return;
	}else {
        $('#fs_mail').css("border", "1px solid #ccc");
    }
    if($('#phonenumber-1').val().length<7){
       $('#phonenumber-1').css("border", "3px solid red");
		return; 
    }else{
        $('#phonenumber-1').css("border", "1px solid #ccc");
    }
    if(check_show_fs_popup==false){
       $('#slide_fs_content').show('slow');
       check_show_fs_popup=true; 
       return;
    }
$("#form_flash_sales").html("<div style='width: 40px;height: 40px;margin: auto; border-right: 3px solid #59c45a;border-bottom: 3px solid transparent;border-radius: 100%;-webkit-animation: spin .9s linear infinite;-moz-animation: spin .9s linear infinite;-o-animation: spin .9s linear infinite;animation: spin .9s linear infinite;'></div><div style='width:100%;float: left;text-align: center;'><p>Please kindly wait a moment...</p></div>");$.ajax({type:"GET",url:SITE_URL+"/ajax-process.php?mod=fs_popup&fullname="+name+"&email="+email+"&phone="+phone+"&date_sale="+datepicker+"&ques="+ques,success:function(xml){var arr_html=xml.split('|');if(arr_html[1]=="ok"){$("#form_flash_sales").html(arr_html[0]);}else{$("#message").html(arr_html[0]);setCookie("popup_flash_sales","1",1);}},error:function(){}});}
...
$(document).ready(function(){ //vuongcm fix
    $('.book-cruise, .book-cruise-st2').click(function(){
        $("html,body").animate({scrollTop:$("#check-rates").position().top-80},2000);
        setTimeout(function(){
    ...
    /*$(document).click(function (e) {
        if (!$("#booking-popup form").is(e.target) && $("#booking-popup form").has(e.target).length === 0) {
            $('#booking-popup').hide();
            $('.check-rates .check-rates-form>form').css('visibility','visible');
        }
    });*/
    $('#booking-popup').on('click',function(){
        $('#booking-popup').hide();
        $('.check-rates .check-rates-form>form').removeAttr('style');
        $('.detail-supplier-cruise .font-caret-up').removeAttr('style');
    });

    $('#review-faci').click(function(){
        $("html,body").animate({
            scrollTop:$("#review-detail").position().top-80},1000);
    });
});
...
window.onload = function() {
    ...
    if($("#modal-sale").length){
        if(getCookie("popup_flash_sales")==""){
            $('#modal-sale').fadeIn();
        }
    }
};
...
function textroom(){
	...
	if(rooms>0){
		...
		str= adults+" Adult(s), "+children+" Children";
	}
	if(infant>0){
		str=str+" ,"+infant+" Infant";
	}
	$('#ip-room').val(str);
...
function ajax_date_flash_deals(departure){
    //var departure = $("#filter-day-cr").val(); 
    $('.ajax-best-selling').html("<p style='width: 100%;float: left;text-align: center;'><img width='70' src='"+SITE_URL_IMG+"load_vtc.gif' /></p>");
    $.ajax({
        type:"GET",
        url:SITE_URL+"/ajax-suplier.php?mod=ajax_date_flash_deals&departure="+departure,
        success:function(xml){
            $('.ajax-best-selling').html(xml);
        }
    });
...
function gui_yeu_cau(room_name){
	...
	if(check_out==undefined){
        ...
        str = str + "Night(s): " + $("#hotel_day").val()+ "\n";
		str = str + "Date Start: " + depar + " Date End: "+check_out;
	}
	$("#f_question").val(str);
	setTimeout(function (){
	$("#f_question").focus();
