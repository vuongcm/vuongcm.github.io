...
function hotel_booking() {
    ...
    if (document.form_booking.check_out.value == d.formatdate("MM/dd/yyyy")){
        cruise_check_err(".div_check_out");
        return;
    }else{
        $(".div_check_out").css("border","none");
    }
    if (document.form_booking.check_in.value == "" || document.form_booking.check_in.value == "Check in"){
        cruise_check_err(".div_check_in");
        return;
...
function tour_booking(){
    ...
    child = $('#child1').val();
    infant = $('#infant1').val();
    if (Number(adult) == 0 && Number(child) == 0) {
        alert("Please fill in number of persons!");
        $("#adult1").parent().css("border","1px solid #ff6600");
        return;
    }
    $("#updating").show();
    document.form_booking.action = document.form_booking.action + "booking.html";
